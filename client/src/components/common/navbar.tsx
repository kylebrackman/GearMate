...
function ResponsiveAppBar() {
  ...
  const [userMenuTrigger, setUserMenuTrigger] =
    React.useState<null | HTMLElement>(null);

  const pages = user
    ? ['Your Rentals', 'List an Item', 'Gear', 'About']
    : ['Gear', 'About', 'Login', 'Sign Up'];
  const dropdown = user
    ? ['Requested Gear', 'Profile', 'Account', 'Dashboard', 'Logout']
    : ['Login', 'Sign Up'];
            ...
            >
              {dropdown.map((option) =>
                option === 'Requested Gear' ? (
                  <MenuItem
                    key="Requested Gear"
                    onClick={() => settingActions['Requested Gear']()}
                  >
                    <Badge
                      badgeContent={
                        user?.received_pending_rental_requests.length
                      }
                      color="secondary"
                      sx={{ display: 'flex' }}
                    >
